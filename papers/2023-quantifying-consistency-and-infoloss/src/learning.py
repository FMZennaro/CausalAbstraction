...
def list_all_alphas_and_errors(A,J=None,errormeasure=None):
    #if errormeasure is None: errormeasure = AbstractionErrorEvaluator
    ...
    for c_alpha in itertools.product(*candidate_alphas.values()):
        ...
        c_A_ev = errormeasure(c_A)

        c_errors.append(c_A_ev.compute_overall_error(J=J))
        c_alphas.append(alphas)
        
    return c_errors,c_alphas

def learn_alpha_by_enumeration(A,J=None,errormeasure=None):
    c_errors,c_alphas = list_all_alphas_and_errors(A,J=J,errormeasure=errormeasure)
